# テキストマップの Minecraft 風 3D 表示化 — 設計指針

## 1. ゴール

* メイン端末（term 0）の **マップ描画領域だけ** を、既存のテキスト/タイル描画から **Godot 3D シーン**（Minecraft 風のボクセル表示）に差し替える。
...
- いずれも **タイル画像をビルボード化（QuadMesh + AlphaCutoff）** して該当ボクセルの上に立てるのが最も実装コストが低い（"Minecraft の絵スプライト" 的見た目）。
- プレイヤーは **三人称俯瞰** で固定し、Camera3D は (player_x, player_y) を追従。Minecraft らしさを優先するなら **斜め見下ろし 45° / FOV 60°** から開始。
