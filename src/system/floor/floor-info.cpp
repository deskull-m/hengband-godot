...
namespace {
/*!
 * @brief ダンジョンサイズ候補テーブル
 * 各要素は (縦ブロック数, 横ブロック数)。実セル数は (SCREEN_HGT * 縦, SCREEN_WID * 横) = (22 * 縦, 66 * 横)。
 *
 * 旧仕様 (1,1)〜(3,3) では 1 ブロック単位が 22×66 (横長 1:3) のためフロアが極端な
 * 横長になりがちだった。MAX_HGT 拡張 (66→198) を機に、全エントリのセル単位アスペクト比を
 * 2:1 以内に収めつつ、サイズが昇順 (calc_blocks == 縦 × 横) で並ぶよう再設計。
 *
 * 面積 (= 縦×横ブロック数) 昇順:
 *  No. (縦, 横)  セル単位 (H×W)   面積  アスペクト
 *   1  (2, 1)    44 ×  66          2   1:1.5
 *   2  (3, 1)    66 ×  66          3   1:1
 *   3  (3, 2)    66 × 132          6   1:2     (横方向の最大アスペクト)
 *   4  (6, 1)   132 ×  66          6   2:1     (縦方向の最大アスペクト)
 *   5  (4, 2)    88 × 132          8   1:1.5
 *   6  (6, 2)   132 × 132         12   1:1     (中規模 正方形)
 *   7  (8, 2)   176 × 132         16   1.33:1
 *   8  (6, 3)   132 × 198         18   1:1.5
 *   9  (8, 3)   176 × 198         24   1:1.125 (最大: 縦横 176 を狙った最大正方形相当)
 */
...
constexpr auto MIN_AREA_BLOCKS = 2; //!< 最小面積ダンジョンのブロック数 (2ブロック = 44×66)
constexpr auto MEDIUM_AREA_BLOCKS = 12; //!< 中面積ダンジョン (12ブロック = 132×132 正方形)
constexpr auto MAX_AREA_BLOCKS = 24; //!< 最大面積ダンジョン (24ブロック = 176×198)
}
