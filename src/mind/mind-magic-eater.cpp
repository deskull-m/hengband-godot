...
bool import_magic_device(PlayerType *player_ptr)
{
    ...
    const auto &[item, i_idx] = choose_object(player_ptr, q, s, USE_INVEN | USE_FLOOR, FuncItemTester(&ItemEntity::can_recharge));
    if (!item) {
        return false;
    }
    ...
    if (tval == ItemKindType::ROD) {
        target_item.count = std::min<byte>(target_item.count + item->number, 99);
    } else {
        for (auto num = item->number; num > 0; num--) {
            int gain_num = pval;
