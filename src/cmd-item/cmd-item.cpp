...
void do_cmd_drop(PlayerType *player_ptr)
{
    ...
    const auto &[item, i_idx] = choose_object(player_ptr, q, s, (USE_EQUIP | USE_INVEN | IGNORE_BOTHHAND_SLOT));
    if (!item) {
        return;
    }
...
void do_cmd_use(PlayerType *player_ptr)
{
    ...
    const auto &[item, i_idx] = choose_object(player_ptr, q, s, options, FuncItemTester(item_tester_hook_use, player_ptr));
    if (!item) {
        return;
    }
...
void do_cmd_activate(PlayerType *player_ptr)
{
    ...
    const auto &[item, i_idx] = choose_object(player_ptr, q, s, (USE_EQUIP | IGNORE_BOTHHAND_SLOT), FuncItemTester(&ItemEntity::is_activatable));
    if (!item) {
        return;
    }
